...
import { GET_SERVICE_TIME_BY_PATH } from '@/lib/queries'
...
interface ServiceTimeByPathData {
  route: {
    entity: DrupalServiceTime
  } | null
}

async function getServiceTime(path: string): Promise<DrupalServiceTime | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<ServiceTimeByPathData>({
      query: GET_SERVICE_TIME_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getServiceTime(path)

  if (!item) {
    return { title: 'Service Time Not Found | Synagogue' }
  }

  return {
    title: `${item.title} | Synagogue`,
    description: ((item as any).body?.processed ? (item as any).body.processed.replace(/<[^>]*>/g, '').substring(0, 160) : '') || `Learn more about ${item.title}.`,
  }
}

export default async function ServiceTimeDetailPage({ params }: PageProps) {
  const { slug } = await params
  const path = `/services/${slug.join('/')}`
  const item = await getServiceTime(path)
                ...
                <dl className="space-y-4">
                  {(item as any).serviceName && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).serviceName}</dd>
                    </div>
                  )}
                  ...
                  {(item as any).serviceStyle && (
                    <div>
                      <dt className="text-sm text-gray-500">Service Style</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).serviceStyle}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
                  <Link
                    href="/contact"
                    className="block w-full text-center px-6 py-3 bg-indigo-700 text-white rounded-lg font-bold hover:bg-indigo-600 transition-colors"
                  >
                    Contact Us
                  </Link>
                </div>
